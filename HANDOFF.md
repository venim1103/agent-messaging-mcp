...
Next action: continue the fixture-only read path by authenticating local client roles in private IPC before wiring pending requests to MCP, then implement trusted extension selection/approval. In-process owner symbols are not client authentication by themselves. Any Gemini send still needs separate approval; the existing command only fills a draft, never clicks Send. A manual user click cannot prove connector-driven submission. Do not auto-retry uncertain sends or treat optimistic UI as delivery. The negotiated VS Code protocol version was not observed.
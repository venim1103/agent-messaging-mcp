...
| --- | --- |
| Repository | Fixture server, WXT popup/background, and a handshake-only native host with a verified browser handshake. An opt-in broker process and private IPC now support pending-only `chat.request_connection`/`chat.get_connection` tools through the official MCP SDK; no extension approval, chat observations, or production send tools exist |
...
- [x] Local IPC foundation: an opt-in broker process atomically owns a `0700` runtime directory with `0600` Unix socket and separate `0600` facade/relay keys. Strict bounded role hellos, per-socket owners, malformed/oversized frame rejection, singleton collision, and process cleanup were tested. An authenticated facade client can request/query pending state; a second client sees `unknown`, a relay gets `PERMISSION_DENIED`. Fifteen companion tests passed, including a two-client MCP stdio -> spawned broker path. The browser-spawned native host still uses only its handshake relay, and the extension has no broker approval or snapshot route. Post-create builds the broker but does not start it.
- [ ] The opt-in broker was started in the current devcontainer with `npm run dev:broker`; the live directory is `0700` and socket/role keys are `0600`. VS Code Chat still listed only the old diagnostic tool after the new SDK build; the pending-only `chat.request_connection`/`chat.get_connection` tools have **not** been invoked through the actual VS Code host. Restart `browserChatFeasibility` and clear cached MCP tools if necessary, then test a pending response without assuming browser approval.
...
Next action: start `npm run dev:broker` only for a bounded VS Code pending-tool check, then extend the browser-spawned relay and trusted extension UI to approve one selected fixture conversation. Do not treat a pending request or a copied handle as browser authorization; snapshot capture and cursors remain unimplemented. Any Gemini send still needs separate approval; the existing command only fills a draft, never clicks Send. Do not auto-retry uncertain sends or treat optimistic UI as delivery. The negotiated VS Code protocol version was not observed.
...
- [x] Local IPC foundation: a broker socket module atomically creates a `0700` runtime directory and `0600` Unix socket, authenticates distinct facade/relay credentials with a strict bounded hello, and mints per-socket owners. Only authenticated facade sockets can create/query pending request state; a second facade sees `unknown`, a relay gets `PERMISSION_DENIED`, and malformed or oversized frames are rejected. Thirteen companion tests passed, including a real socket test. No process yet provisions the role keys or starts this broker from the MCP facade or browser-spawned native relay; no chat data is available over the socket.

...
| --- | --- |
| Repository | Fixture server, WXT popup/background, and a handshake-only TypeScript native host with a verified browser handshake; diagnostic-only MCP stdio tool called through a local client and VS Code Chat. Internal private broker IPC and pending-request handling have socket tests, but no running broker process or chat MCP tools exist yet |
...
- [x] Milestone 1 internal foundation: a companion-only pending-request registry mints UUIDs, accepts only in-process owner symbols, caps active requests at 100, reports expiry after 60 seconds, and retains expired status for one more minute. Owner isolation, capacity, expiry, and disconnect cleanup tests passed. It is not connected to an authenticated broker, MCP tools, native IPC, or the browser.
- [x] Local IPC foundation: a broker socket module atomically creates a `0700` runtime directory with `0600` Unix socket and separate `0600` facade/relay key files, authenticates strict bounded hellos, and mints per-socket owners. Only authenticated facade sockets can create/query pending request state; a second facade sees `unknown`, a relay gets `PERMISSION_DENIED`, and malformed or oversized frames are rejected. Thirteen companion tests passed, including a real socket and role-file test. This provisioning runs only inside tests; no production process starts the broker or gives the distinct role keys to the MCP facade/native relay, and no chat data is available over IPC.
...
Next action: securely provision short-lived facade/relay role credentials to their distinct processes, bootstrap the broker without exposing a network listener, then wire the facade's pending-request MCP tools on the local fixture. The in-process test credentials and owner symbols are not a live client-authentication system. Trusted extension selection/approval and any chat snapshots remain unimplemented. Any Gemini send still needs separate approval; the existing command only fills a draft, never clicks Send. Do not auto-retry uncertain sends or treat optimistic UI as delivery. The negotiated VS Code protocol version was not observed.
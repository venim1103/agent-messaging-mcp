...
The Gemini read-only popup inspects only the user-selected tab and returns fixed structural indicators and bounded message character counts; it does not return message text, draft contents, or conversation identifiers. It is safe to share its complete output for diagnostics, but it does not grant a persistent chat connection or send anything.

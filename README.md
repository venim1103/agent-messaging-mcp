...
Milestone 0 has a synthetic chat fixture, a read-only WXT popup, a handshake-only Native Messaging host, and a diagnostic-only official-SDK MCP stdio server. In container Chromium, the user inspected `fixture-alpha` and both rendered messages through the popup and saw `Native bridge ready (protocol v1)`; no message was sent. A local MCP client called the diagnostic tool, but VS Code host compatibility is still unverified. There is no broker, chat MCP integration, or Gemini adapter yet.
...
VS Code's workspace [MCP configuration](.vscode/mcp.json) starts the diagnostic server with the container's Node executable and built companion entry point. Start `browserChatFeasibility` using **MCP: List Servers**, then call `browser_chat_feasibility` in VS Code Chat. It returns a fixed no-browser-data result; it is not the chat connector and has not yet been verified through VS Code itself.

...
Milestone 1 has an internal private Unix broker socket and owner-bound pending connection requests, tested with separate facade and relay clients. The tested broker module creates private role-key files, but no process currently starts it or supplies those credentials to the MCP facade or native host. No MCP chat tool or browser message access is available through this IPC foundation.
